 ...
 *
 * Поддерживает компоненты Bootstrap (Нужно передать в wv_nav_menu(['wv_bootstrap_type_component])):
 *  -   Dropdown
 *  -   Collapse
 */
class Walker_Bootstrapped_Menu extends Walker_Nav_Menu {
	...
	function start_lvl( &$output, $depth = 0, $wv_index_top_lvl_element = 0, $args = null ) {
		$bs_type       = $args->wv_bootstrap_type_component;
		...
		$bs_type . ( 'dropdown' === $bs_type ? '-menu' : '' ) .
		'" id="' . $bs_type . '-' .
		$args->menu_id . '-' .
		$args->menu->term_id . '-' . $wv_index_top_lvl_element .
		'" class="' . $bs_type . '" ' .
		// 'data-bs-parent="#' . $args->menu_id .
		'>' . "\n";
	}
	...
	function start_el( &$output, $data_object, $wv_index_top_lvl_element = 0, $depth = 0, $args = null, $current_object_id = 0 ) {
		...
		$item_output = '';
		// if ( $depth === 0 && ( 'dropdown' !== $bs_type ) ) {
		// $item_output = '<div class="menu-item-header">';
		// }
		if ( $depth === 0 ) {
			$item_output .= sprintf(
				'<button class="wv-universal-toggle" ' .
				'data-bs-toggle="' .
				$bs_type . '" data-bs-target="#' . $bs_type . '-%s"></button>',
				$args->menu_id . '-' . $args->menu->term_id . '-' . $wv_index_top_lvl_element
			);
		}
